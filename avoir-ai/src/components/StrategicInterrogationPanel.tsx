...
export default function StrategicInterrogationPanel({ baseDirective, onComplete, onCancel }: StrategicInterrogationPanelProps) {
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: 20 }}
      className="absolute bottom-full left-0 right-0 mb-4 z-50 p-6 rounded-2xl bg-card/95 border border-warning shadow-[0_0_50px_rgba(245,158,11,0.15)] backdrop-blur-xl mx-4"
    >
      <div className="absolute top-0 left-0 right-0 h-1 bg-card rounded-t-2xl overflow-hidden">
        <motion.div
      ...
      <div className="flex justify-between items-start mb-6 mt-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-bold font-tactical tracking-wider text-warning">STRATEGIC INTERROGATION MODE</h3>
            <p className="text-xs text-muted-foreground">Your prompt was too vague. Let's sharpen the blade.</p>
          </div>
        </div>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="text-lg font-bold text-foreground">{currentQ.title}</h4>
          ...
          <div className="relative group">
            <input
              ...
              className="w-full bg-background border border-border focus:border-warning rounded-xl px-4 py-3 text-foreground placeholder-zinc-600 outline-none transition-all focus:ring-1 focus:ring-amber-500/50"
            />
            <button
              onClick={handleNext}
              disabled={!answers[currentQ.key as keyof typeof answers].trim()}
              className="absolute right-2 top-1/2 -translate-y-1/2 p-2 rounded-lg bg-warning text-black disabled:opacity-30 disabled:bg-muted disabled:text-muted-foreground hover:bg-warning transition-colors"
            >
