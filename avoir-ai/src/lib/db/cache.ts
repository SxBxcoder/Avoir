...
function getRedis(): Redis | null {
  ...
  if (!url || !token) {
    logger.warn('db.cache', 'Redis env not set, cache disabled');
    return null;
  }
...
export async function getCachedQuota(userId: string): Promise<{ used: number; limit: number } | null> {
  ...
  try {
    ...
    logger.warn('db.cache', 'Redis read failed', { err });
    return null;
  }
}
...
export async function setCachedQuota(userId: string, used: number, limit: number): Promise<void> {
  ...
  try {
    ...
    logger.warn('db.cache', 'Redis write failed', { err });
  }
}
...
export async function incrementCachedQuota(userId: string): Promise<number | null> {
  ...
  try {
    ...
    logger.warn('db.cache', 'Redis increment failed', { err });
    return null;
  }
}
...
export async function checkRateLimit(
  ...
  try {
    ...
    logger.warn('db.cache', 'Rate limit check failed', { err });
...
export async function getCachedCampaign(goalHash: string): Promise<any | null> {
  ...
  try {
    ...
    logger.warn('db.cache', 'Campaign cache read failed', { err });
    return null;
  }
}
...
export async function setCachedCampaign(goalHash: string, data: any): Promise<void> {
  ...
  try {
    ...
    logger.warn('db.cache', 'Campaign cache write failed', { err });
  }
}
...
export async function getCachedAlphaBrief(): Promise<AlphaBrief | null> {
  ...
  try {
    ...
    logger.warn('db.cache', 'Alpha brief read failed', { err });
    return null;
  }
}
