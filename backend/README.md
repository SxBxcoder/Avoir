...
- First call of the day generates the brief via Gemini (`gemini-3-flash-preview`) and caches it until local midnight.
- Subsequent calls serve the cached copy (zero LLM latency).
- `?force=true` bypasses the cache and regenerates.
